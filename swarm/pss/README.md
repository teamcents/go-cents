...
The code tutorial [p2p programming in go-cents](https://github.com/nolash/go-cents-p2p-demo) by [@nolash](https://github.com/nolash) provides step-by-step code examples for usage of `pss` API with `go-cents` nodes.
...
For `golang` clients, please use the `rpc.Client` provided by the `go-cents` repository. The return values may have special types in `golang`. Please refer to `godoc` for details.
